...
namespace BVSSClosenessKernels
{
    ...
    __global__ void BVSSCloseness8EnhancedSliceSize8NoMasks4Lazy    (
                                                                    ...
                                                                    // current
                                                                    unsigned*                   const   __restrict__ far,
                                                                    ...
                                                                    )
    {
        ...
        while (cont)
        {
            ...
            for (unsigned i = warpID; i < currentFrontierSize; i += noWarps)
            {
                ...
                if (laneID < 8)
                {
                    frontierVertex = frontier[laneID * partitionSize + rset]; // very bad access
                }
                else
                {
            ...
            for (unsigned vBase = threadID << 3; vBase < paddedN; vBase += noThreads << 3)
            {
                ...
                for (unsigned vertex = vBase; vertex < vBase + 8; ++vertex)
                {
                    ...
                    if (totalSet != 0)
                    {
                        ...
                        far[vertexIndex] += levelCount * totalSet;
                                                                                ...
                                                                                )
    {
        ...
        while (cont)
        {
            ...
            for (unsigned i = warpID; i < currentFrontierSize; i += noWarps)
            {
                ...
                for (unsigned chunk = 0; chunk < 256; ++chunk)
                {
